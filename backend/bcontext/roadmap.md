...
| 0.2 | **Supabase + PostGIS** | Connection through `app/lib/database.py` (SQLAlchemy, async). PostGIS extension enabled. Alembic initialised. First migration: `scenes`, `investigations`, `runs`, `evidence`, `claims`, `trace_steps`, `missions` with geometry columns. | A round trip that writes a polygon and reads it back with a correct area in an equal-area projection. | todo |
| 0.3 | **Redis (docker)** | `app/lib/redis.py`. Two uses, kept separate: model-manager locks and short-lived cache. Async client. | Set/get round trip; a lock held by one process blocks a second and releases on crash. | todo |
| 0.4 | **MinIO (docker)** | `app/lib/storage.py` over the S3 API, async. Buckets `raw`, `cog`, `artefacts`, `figures`, `reports`. Presigned PUT/GET. | A file uploaded through a presigned URL is readable back by key; CORS configured — including on `figures`, which the browser loads cross-origin as `<img>` (`api-contract.md` §6). | todo |
...
| 0.6 | **`aeris doctor`** | One CLI command printing a table: every dependency, reachable yes/no, version, latency, and the config values in force (secrets masked). | `aeris doctor` is green on a clean machine after `docker compose up`. **This is the command the product owner runs to verify a setup.** | todo |
| 0.7 | **Contract vendoring** | A script exports the frontend's Zod schemas to JSON Schema into `bcontext/contracts/`. A pytest validates backend fixtures against them. | A deliberately wrong field name fails the contract test. | todo |

**Phase 0 gate:** `docker compose up` then `aeris doctor` — all green, on a machine that has never run this
project.
